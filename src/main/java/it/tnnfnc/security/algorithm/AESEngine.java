 ...
 * 
 */
public final class AESEngine implements I_BlockCipher {
	// The S box
	...
	private int[][] generateWorkingKey(byte[] key, boolean forEncryption) {
		...
		if (((KC != 4) && (KC != 6) && (KC != 8)) || ((KC * 4) != key.length)) {
			throw new IllegalArgumentException(
					"KeyboardKey length not 128/192/256 bits.");
		}
	...
	public void init(boolean forEncryption, AlgorithmParameters params) {
		if (params.get(KeyAlgorithmParameter.class) instanceof KeyAlgorithmParameter) {
			// System.out.println("AESEngineImpl "
			// + ((KeyAlgorithmParameter) params
			// .get(KeyAlgorithmParameter.class)));
			WorkingKey = generateWorkingKey(
					((KeyAlgorithmParameter) params.get(KeyAlgorithmParameter.class))
							.getKey(), forEncryption);
